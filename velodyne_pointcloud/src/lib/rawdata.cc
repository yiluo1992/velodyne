  ...
  for (int i = 0; i < BLOCKS_PER_PACKET; i++)
  {
    ...
    for (int j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE)
    {
      ...
      if ((raw->blocks[i].rotation >= config_.min_angle && raw->blocks[i].rotation <= config_.max_angle && config_.min_angle < config_.max_angle) || (config_.min_angle > config_.max_angle && (raw->blocks[i].rotation <= config_.max_angle || raw->blocks[i].rotation >= config_.min_angle)))
      {
        ...
        intensity += focal_slope * (abs(focal_offset - 256 * (1 - static_cast<float>(tmp.uint) / 65535) * (1 - static_cast<float>(tmp.uint) / 65535)));
...
/** @brief convert raw packet to point cloud
   ...
   *  @param pkt raw packet to unpack
   *  @param pc shared pointer to point cloud (points are appended)
   *  @param cpi current_packect_index, 0 based
   *  @param nppm num_packet_per_msg
   */
void RawData::unpack(const velodyne_msgs::VelodynePacket &pkt,
                     VPointCloud &pc, int cpi, int nppm)
{
  ROS_DEBUG_STREAM("Received packet, time: " << pkt.stamp);
  ...
  for (int i = 0; i < BLOCKS_PER_PACKET; i++)
  {
    ...
    for (int j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE)
    {
      ...
      if ((raw->blocks[i].rotation >= config_.min_angle && raw->blocks[i].rotation <= config_.max_angle && config_.min_angle < config_.max_angle) || (config_.min_angle > config_.max_angle && (raw->blocks[i].rotation <= config_.max_angle || raw->blocks[i].rotation >= config_.min_angle)))
      {
        ...
        intensity = int(intensity) + time_correction_in_msg;
...
/** @brief convert raw packet to point cloud
   ...
   */
void RawData::unpack(const velodyne_msgs::VelodynePacket &pkt,
                     pcl::PointCloud<PointXYZData> &pc, int cpi, int nppm)
{
  ROS_DEBUG_STREAM("Received packet, time: " << pkt.stamp);
  ...
  if (calibration_.num_lasers == 16)
  {
    std::cout << "VLP16 not support yet!\n";
    return;
  }
